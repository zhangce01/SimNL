    ...
    for set in all_dataset:
        ...
        for k in k_shot:
            ...
            if set == 'imagenet':
                ...
                dataset = ImageNet_N(cfg['root_path'], cfg['shots'], preprocess)
                val_loader = torch.utils.data.DataLoader(dataset.test, batch_size=64, num_workers=8, shuffle=False)
                train_loader_cache = torch.utils.data.DataLoader(dataset, batch_size=256, num_workers=8, shuffle=False)
                # print(dataset.train.targets)
                for i, (images, targets) in enumerate(train_loader_cache):
                    print("Batch targets:", targets)
                    break  # Check the first batch only for simplicity
            else:   
        ...
        if not set == 'imagenet' and not set == 'imagenet_n':
