...
from datasets.imagenet_n import ImageNet_N
...
def SimNL(cfg, cache_keys, cache_values, val_features, val_labels, test_features, test_labels, clip_weights_template, clip_weights_cupl, clip_weights_negative, clip_model, train_loader_F):
...
def main():
    ...
    if test_dataset is None:
        ...
        if cfg['dataset'] == 'imagenet' or cfg['dataset'] == 'imagenet_n':
    ...
    elif cfg['dataset'] == 'imagenet_n':
        imagenet_n = ImageNet_N(cfg['root_path'], cfg['shots'], preprocess)
        train_loader_F = torch.utils.data.DataLoader(imagenet_n, batch_size=256, num_workers=8, shuffle=True)
    else:   
    ...
    SimNL(cfg, cache_keys, cache_values, val_features, val_labels, test_features, test_labels, clip_weights_template, clip_weights_cupl, clip_weights_negative, clip_model, train_loader_F)
